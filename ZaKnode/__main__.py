...
def run():
    ...
    my_game.addFont("default", my_game.directory("../joy/orbital-defender/img/early_gameboy.ttf"))
    ...
    desc1 = nodes.Label(default, "Use <- / -> to switch scenes", my_game.fonts["default"]["l"], offset_str="center")
    press_esc = nodes.Label(default, "Press ESC to exit", my_game.fonts["default"]["m"], offset_str = "bottom-right")
    ...
    desc = nodes.TextBlock(scene1, "Visible axis can change objects position", my_game.fonts["default"]["s"], offset_str="bottom", offset = Vector2(0, -120))
    desc_1 = nodes.TextBlock(scene1, "Red line shows the path of AxisMove", my_game.fonts["default"]["s"], offset_str="bottom", offset = Vector2(0, -80))
    desc1 = nodes.Label(scene1, "Showcase of linear and gradual interpolation on block of color", my_game.fonts["default"]["l"], offset_str="top")
    ...
    explain_text = nodes.Label(scene4, "Explanation: ", my_game.fonts["default"]["s"], (200, 200, 200), 8, offset_str = "top-right", offset = Vector2(-20, 20))
    ...
    desc5 = nodes.Label(scene4, "Translation of one tile of a tilemap \nfrom 1 point to another", my_game.fonts["default"]["l"], (220, 240, 190), offset = Vector2(130, 900))
    ...
    for i in range(int(my_game.size.x) // block_size):
        ...
        if i == 0:
            modifiers.AxisMove(new_block, start = 200, end = my_game.size.y - block_size - 200, axis = "y", mode = "ease-both", speed = 1000, strength = 1.5)
        else:
    ...
    combined_offset = 5
    for font_key in list(my_game.fonts["default"].keys()):
        last_block = nodes.TextBlock(scene10, f"Font sizes {font_key}", my_game.fonts["default"][font_key], txt_color = (50, 240, 230), bg_color = (80, 100, 90), padding = 9, offset_str = "top", offset = (0, combined_offset))
